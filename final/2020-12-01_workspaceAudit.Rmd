...
include.me = paste0(path.github, "humanVerseWSU/R/functions-system.R");
source_url( include.me );
...
```{r}
wsu.id = "nic_trout"; # change this to yours.
github.repository = "https://raw.githubusercontent.com/njtrout/WSU_STATS419_FALL2020/";
...
```{r}
path.to.git.local = "C:/Users/Nic Trout/Documents/C/WSU_STATS419_FALL2020/";
...
for(i in 1:total.commits)
  ...
  for(j in 1:3)
    {
    if(list.element.exists(my.detail,j))
      {
      val = as.numeric( my.detail[[j]][1] );
      key = substr( my.detail[[j]][2],1,1 );
      k = switch(key,
          "f" = 1,
			    "i" = 2,
			    "d" = 3,
