...
The MNIST database is made up of handwritten digits. In the database there is a training set of 60,000 examples and 10,000 test examples from 250 people. This data was created and taken in November of 1998 by Yann LeCun, Corinna Cortes, and Christopher J.C. Burges. This data was used to test and train classifiers like K-NN, SVM, and many more.  
...
These numbers above are the accuracy of our predictions. Meaning the percent mean preformed the worse only being 21% accurate and the linear combination of different independent approaches performed the best at 29%.
...
No I don't think there is a way to improve this existing algorithm.

- Should we truncate the eigenvalues from 28 and remove all of the "zeroes" ... is that creating a problem?

I don't think that that would solve the issue. I think that the classifier needs to be trained on multiple smaller matrices and then it would detect smaller changed in numbers. Then maybe a squiggly one will not have similar eigenvalues as the number eight.
