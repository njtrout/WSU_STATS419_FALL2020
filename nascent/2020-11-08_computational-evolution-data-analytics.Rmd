...
This algorithm grabs the first two pairs of numbers and compares them. It them puts it in the right position based on if it is greater or not in a new vector. Then it grabs the next number and compares it to those numbers to see if it should go before or after and then stores that in the new vector and so on. It was probably called laundry sorting because it grabs items and sorts them as you would sort laundry.
...
I think a good stopping rule would be to stop once the new bucket is the same length as the original, therefore, all the numbers would be sorted and stop when relevant. 
...
```
This algorithm goes through and find the least value and store it as least. Then loop through and find the greatest value and store it as the greatest. Then continue to loop through finding mins and maxs till all values are looped over.
...
For bubble sort you loop through the numbers comparing the first and second number to see which is bigger. If the first number is bigger then swap, otherwise leave it. Continue looping until no more swaps are needed.
...
I think its fair to say that sort is the fastest and most relevant. On this graph you can see that as you sort more the time in seconds is the least for sort and it stays that way as the number to sort increases.
