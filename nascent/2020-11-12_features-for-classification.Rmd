...
It appears that some of the numbers in the training data set such as 1, 2, 6, 8, and 0, all seem to look similar and may portray similar eigen values. With that I think that this classifier may not end up being that accurate when training. I think that more variability would be better when training this classifiers.
...
getting proportions of sub matrices 
...
## matrix.features            
sub = wildcardSearch( paste0("*_","gridcount"), "data.id", df=sub);
## comparison.methods
sub = subsetDataFrame( sub, "comparison.method", "==" , "sim.cosine");
## decision.stats
result = list();
...
Dividing the matrix by 6 and using the gridcount method I was able to improve my top 2 which became the best at .6. I would divide the martix by more so you could better differentiate between egien values.
