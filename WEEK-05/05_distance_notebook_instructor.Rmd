...
nyc = as.data.frame( matrix( c(  nyc.timesquare.latlong,
                    ...
                    ,ncol=2,byrow=TRUE) );
  nyc.places = c("Times Square", "The Bull on WallStreet", "The Lincoln Center", "Macy's", "Broadway (Les Miserable)", "St. Patrick's", "Di Fara Pizza", "Molly's Cupcakes", "Saks 5th Avenue");
  rownames(nyc) = nyc.places;
...
the angle would matter when it the angle of rotation is 90 degrees
...
You would have to use a different method as the map goes from being 90 degree angles to a grid shape that is different and not 90 degrees.
...
calculate actual vs each method,

actual distance from "Time Square" to "The Bull on WallStreet" is 4.2 miles according to google and when running the code below you will see Haversine is the most accurate of the three methods listed, measuring the distance at 3.903537 
```{r, nyc-convert}


nyc.haversince = round(conv_unit(  distm( nyc[,2:1], fun=distHaversine),  "m", "mi"), digits = 2); # meters to miles
rownames(nyc.haversince) = nyc.places;
colnames(nyc.haversince) = nyc.places;
nyc.haversince;

nyc.meeus = round(conv_unit(  distm( nyc[,2:1], fun=distMeeus),  "m", "mi"), digits = 2); # meters to miles
rownames(nyc.meeus) = nyc.places
colnames(nyc.meeus) = nyc.places

nyc.geo = round(conv_unit(  distm( nyc[,2:1], fun=distGeo),  "m", "mi"), digits = 2); # meters to miles
rownames(nyc.geo) = nyc.places
colnames(nyc.geo) = nyc.places
...
#converting neighbors distance from m to mi and measuring with distharvesine and creating dataframe of data
result.neighbors$distance = conv_unit(distm(result.neighbors[,3:2], fun=distHaversine),  "m", "mi")[,6];
#creating a limit on distance < than 10
result.neighbors$distance = result.neighbors$distance <= 10;
#removing the specified zip code
result.neighbors$zipcode = result.neighbors[result.neighbors$zipcode!='99163',];
