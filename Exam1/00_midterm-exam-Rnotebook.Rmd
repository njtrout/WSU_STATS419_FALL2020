...
This boxplot is of job.count on the x and frequency on the y. This shows you how many time the jobs are searched. In this case the jobs.subset histogram shows the number of these jobs related to our degree are searched, it shows some searched 50,000-100,000 times but majority of the jobs listed are searched 0-50,000 times in the 5 week period we are looking at.
...
This boxplot is comparing the number of searches being done for C++ and Microsoft Office. The C++ boxplot appears to have a an average search of a little over 160k searches with a min of 159k and a max of about 175k. The Microsoft boxplot shows a min number of searches of about 225k, the mean is at the top of the IQR now at about 232k. Microsoft appears to have no max as there is an extreme outliar just shy of 240k. 
...
This boxplot shows that Java has a higher median, statistic has a lower minimum and higher maximum. The IQR does overlap some but overall I would say Java is searched more.
...
I would say overall that big data is searched more than data science. Based on the first plot there is more searches done on big data all the way until the last week. That proves true in the boxplot as the mean is quite higher for big data than data science.
...
This box is not a square because in your buildBoundingBoxFromRadiusAndGivenLatitudeLongitude function you specify your latitude and longitude to make it a rectangle. If I were to try to make it better I would make sure all dots were in the rectangle and not have any on the boarder where it is hard to see. I might also maybe pick a red/white color combo that sticks out more and easier to read.
...
hometown.info = getNeighborsFromLatLong(13, 48.0518, -122.1771, "mi");
...
Visually I would say the first plot is the most appealing to me as it seems the most friendly to read and to relay information.

Functionally, I still think the first plot is the most effective as I don't not fully understand what the second plot is portraying.

One key factor that would determine the appropreatness would be if we were looking for similar trends between those states. 
...
dist.meeus = conv_unit(  distm( latlong[,1:2],
...
dist.haversine = conv_unit(  distm( latlong[,1:2],
