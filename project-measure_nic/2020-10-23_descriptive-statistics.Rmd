...
{
  measure$data_collector = factor(measure$data_collector);
  measure$person_id = factor(measure$person_id);
  measure$writing = factor(tolower(measure$writing));
  measure$eye = factor(tolower(measure$eye));
  measure$`eye_color` = factor(tolower(measure$`eye_color`));
  measure$ethnicity = factor(tolower(measure$ethnicity));
  ...
  measure$side = factor(tolower(measure$side));
  ...
  #measure$units[measure$units=="in"] = conv_unit(measure, "inch", "cm");
  measure[measure$units=="cm", ][4:26]=conv_unit(measure[measure$units=="cm", ][4:26], "cm", "inch")
  ...
  #View(measure)
  #removing unneeded/unused columns
  #hand length, hand width, weight, wingspan, height, standing, reach 
  ...
  measure.df = subsetDataFrame(measure, "gender", "==", "male")
  measure.df = subsetDataFrame(measure.df, "age", ">=", "18")
  measure.df = subsetDataFrame(measure.df, "age", "<=", "40")
  #View(measure.df)
  
  
  measure.ndf = measure.df[, c(21,22,4,24,7,14)];
  is.num = sapply(measure.ndf, is.numeric);
  measure.ndf[is.num] = lapply(measure.ndf[is.num], round, 2);
  measure.ndf = cbind(measure.df$person_id, measure.ndf);
  
  measure.ndf = na.omit(measure.ndf);
  set.seed(123)
  rows=sample(1:(nrow(measure.ndf)),75, replace = TRUE, prob = NULL)
  measure.fdf = measure.ndf[rows,]
  mynames = c("person", "hand.length", "hand.width", "height", 
              "arm.reach", "arm.span", "age")
  colnames(measure.fdf) = mynames
...
sd_of_data = sapply(another.example, sd, na.rm = TRUE);
View(sd_of_data);
################################################################################
