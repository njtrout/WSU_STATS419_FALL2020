...
```{r, mychunck-common-include, message=FALSE}
library(devtools) 
my.source = 'local';
...
#rotateMatrix90(mat)
rotate <- function(myMatrix) t(apply(myMatrix, 2, rev))
rotate(myMatrix)
					
#rotateMatrix180(mat)
rotate <- function(myMatrix) t(apply(myMatrix, 2, rev))
rotate(rotate(myMatrix))

#rotateMatrix270(mat)
rotate <- function(myMatrix) t(apply(myMatrix, 2, rev))
rotate(rotate(rotate(myMatrix)))
...
# Will vs Denzel 
```{r, mychunck-imdb-include, message=FALSE}
source(paste0(local.path, "functions/functions-imdb.R"), local=T );
```

Question 6: Compare Will Smith and Denzel Washington. [See 03_n greater 1-v2.txt for the necessary functions and will-vs-denzel.txt for some sample code and in DROPBOX: \__student_access__\unit_01_exploratory_data_analysis\week_02\imdb-example ]  You will have to create a new variable $millions.2000 that converts each movie's $millions based on the $year of the movie, so all dollars are in the same time frame.  You will need inflation data from about 1980-2020 to make this work.
