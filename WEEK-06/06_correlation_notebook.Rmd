...
x.9.1 = rnorm(100,9,1);   normalDiagnosticPlot(x.9.1, showSDs=TRUE);
...
t.test(x.0.1, x.0.1b, alternative = "two.sided", var.equal=FALSE);
t.test(x.0.1, x.0.1b, alternative= "greater", mu=0);
...
```{r, chunck-welch-t}
t.test(x.0.1, x.0.1, alternative= "greater", mu=0);
t.test(x.0.1, x.0.1b, alternative= "greater", mu=0);
t.test(x.0.1, x.3.1, alternative= "greater", mu=3);
...
The negative and positive slope is because one has negative correlation and one has positive correlation. 1 means perfect positive correlation. 
...
rank and minutes have a weak negative correlation 

##### millions vs. year 
--WRITE SOMETHING HERE--
millions and year have a weak positive correlation

##### ratings vs. adjMillions2000 
--WRITE SOMETHING HERE--
ratings and adjmillions2000 have a slight negative correlation 
...
Correlation checks linear dependency and correlation does not imply independence because it does not tell if two things depend on each other.
