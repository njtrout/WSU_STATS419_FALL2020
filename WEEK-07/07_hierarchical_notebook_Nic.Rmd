...
protein = read.table(protein.file, sep = "|");
row = protein[1,];
...
# why would I do this?
#protein.file2 = paste0(example.datasets.path,"protein2.txt");
...
Yes Romania, Bulgaria, and Yugoslavia all have similar protein eating habits based on the above clusters. The clusters in most of the dendograms are consistent and similar when it comes to grouping, such as Italy and Greece.
...
protein.t[] = lapply(protein.t,function(x) as.numeric(as.character(x)))
for(method in methods)
  {
  time.start = Sys.time();
...
I would say that yes the Portugal appears to be the odd country out.
...
I would say yes I need to scale this data because when looking at the original data, the protein consumption does not add to 100%.
...
protein = protein[,-1]
View(protein)
protein = transposeMatrix(protein)
...
for(method in methods)
  {
  time.start = Sys.time();  
      X.hclust = hclust( dist(protein), method=method);
  plot(X.hclust);
  time.end = Sys.time();
  elapse = sprintf("%.3f", as.numeric(time.end) - as.numeric(time.start));
  print(paste0(elapse, " secs to complete method ... ", method));
...
for(method in methods)
  {
  time.start = Sys.time();
      X.pvclust = pvclust ( protein, method.hclust=method);
...
I used X to perform the above analysis
...
I would do clusters of body parts to see what body parts are outliers rather than participants. Participants would be quite linear I would think as it would cluster people who are likely to be the same together.



## Dataset `cars` 

```{r, chunck-load-cars-monte-version}

# https://www.webpages.uidaho.edu/~stevel/519/Data/MBA_CAR.txt

car.file = paste0(example.datasets.path,"MBA_CAR_ATTRIB.txt");

mba.cars = na.omit(read.table(car.file,na.strings='.', header = FALSE));
...
X.hclust.info = perform.hclust(X, 8);
str(X.hclust.info);
...
Hclust above and pvclust below
```{r}
cars[] = lapply(cars,function(x) as.numeric(as.character(x)))
