...
path.to.week7 = "C:/Users/Nic Trout/Documents/C/WSU_STATS419_FALL2020/WEEK-07/";
...
example.datasets.path = "C:/Users/Nic Trout/Documents/C/WSU_STATS419_FALL2020/datasets/";




protein.file = paste0(example.datasets.path,"protein.txt");
protein = read.table(protein.file,sep = "|", header = TRUE);
...
To do so you would do k means clustering of the countries that have similar protein intake. That will show me which countries have similar protein consumption habits. I could see above that Finland, Germany, and Sweeden have similar eating habits.
...
When looking at the Hartigan-Wong method, I think the optimal Kmeans value would be Kmeans = 4 based on the chart showing K vs time.
...
When looking at the Lloyd method, I think the optimal Kmeans value would be Kmeans = 3 based on the chart showing K vs time. 
...
When looking at the Forgy method, I think the optimal Kmeans value would be Kmeans = 3 based on the chart showing K vs time. 
```{r}
...
When looking at the MacQueen method, I think the optimal Kmeans value would be Kmeans = 2 based on the chart showing K vs time. 
```{r}
knitr::include_graphics("4.png")
```
Overall, when looking at the optimal K based on the K vs time chart, you will see they all have similar results and Kmeans=3 seems like that would be the best overall choice.
...
I think based on the final report it shows red meat, cereal, and fruits and vegies as most consumed types of foods.

## SO WHAT: Review implications

These results are limited because the data was collected in Year 1981 before the fall of the USSR (Soviet Union).  Eating habits may have changed.
...
I think yes you should remove USSR to see if the data was skewed or not when that data is included.
...
I think an interesting follow-up research question would be to look the different protein consumptions in the US vs Russia.
